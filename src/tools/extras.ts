...
export function registerDiscussionTools(server: McpServer) {
  ...
  tool(server, "view_forum_category", "View a forum category.", { category_id: z.number().int() }, async ({ category_id }) => {
  ...
  tool(server, "create_forum_category", "Create a forum category.", { category: z.record(z.any()) }, async ({ category }) => {
  ...
  tool(server, "update_forum_category", "Update a forum category.", { category_id: z.number().int(), category: z.record(z.any()) }, async ({ category_id, category }) => {
  ...
  tool(server, "delete_forum_category", "Delete a forum category.", { category_id: z.number().int() }, async ({ category_id }) => {
  ...
  tool(server, "list_forums", "List forums under a category.", { category_id: z.number().int() }, async ({ category_id }) => {
  ...
  tool(server, "view_forum", "View a forum.", { forum_id: z.number().int() }, async ({ forum_id }) => {
    const res = await fd.get(`/discussions/forums/${forum_id}`);
    return text(res.ok ? res.data : errorPayload("Failed to view forum", res));
  });
  tool(server, "create_forum", "Create a forum.", { category_id: z.number().int(), forum: z.record(z.any()) }, async ({ category_id, forum }) => {
  ...
  tool(server, "update_forum", "Update a forum.", { forum_id: z.number().int(), forum: z.record(z.any()) }, async ({ forum_id, forum }) => {
  ...
  tool(server, "delete_forum", "Delete a forum.", { forum_id: z.number().int() }, async ({ forum_id }) => {
    const res = await fd.delete(`/discussions/forums/${forum_id}`);
    if (res.status === 204) return text({ success: true });
    return text(errorPayload("Failed to delete forum", res));
  });

  tool(server, "list_topics", "List topics under a forum.", { forum_id: z.number().int() }, async ({ forum_id }) => {
  ...
  tool(server, "view_topic", "View a topic.", { topic_id: z.number().int() }, async ({ topic_id }) => {
    const res = await fd.get(`/discussions/topics/${topic_id}`);
    return text(res.ok ? res.data : errorPayload("Failed to view topic", res));
  });
  tool(server, "create_topic", "Create a topic.", { forum_id: z.number().int(), topic: z.record(z.any()) }, async ({ forum_id, topic }) => {
  ...
  tool(server, "update_topic", "Update a topic.", { topic_id: z.number().int(), topic: z.record(z.any()) }, async ({ topic_id, topic }) => {
  ...
  tool(server, "delete_topic", "Delete a topic.", { topic_id: z.number().int() }, async ({ topic_id }) => {
    const res = await fd.delete(`/discussions/topics/${topic_id}`);
    if (res.status === 204) return text({ success: true });
    return text(errorPayload("Failed to delete topic", res));
  });

  tool(server, "list_topic_comments", "List comments under a topic.", { topic_id: z.number().int() }, async ({ topic_id }) => {
  ...
  tool(server, "create_topic_comment", "Create a topic comment.", { topic_id: z.number().int(), comment: z.record(z.any()) }, async ({ topic_id, comment }) => {
  ...
  tool(server, "update_topic_comment", "Update a topic comment.", { comment_id: z.number().int(), comment: z.record(z.any()) }, async ({ comment_id, comment }) => {
  ...
  tool(server, "delete_topic_comment", "Delete a topic comment.", { comment_id: z.number().int() }, async ({ comment_id }) => {
...
export function registerSurveyTools(server: McpServer) {
  ...
  tool(server, "view_survey", "View a survey.", { survey_id: z.number().int() }, async ({ survey_id }) => {
...
export function registerAutomationTools(server: McpServer) {
  ...
  tool(server, "view_automation_rule", "View an automation rule.", {
    automation_type_id: AutomationTypeIdEnum, rule_id: z.number().int(),
  ...
  tool(server, 
    ...
    { automation_type_id: AutomationTypeIdEnum, rule_id: z.number().int(), rule: z.record(z.any()) },
  ...
  tool(server, "delete_automation_rule", "Delete an automation rule.", {
    automation_type_id: AutomationTypeIdEnum, rule_id: z.number().int(),
  }, async ({ automation_type_id, rule_id }) => {
...
export function registerScenarioTools(server: McpServer) {
  ...
  tool(server, "view_scenario_automation", "View a scenario automation.", { scenario_id: z.number().int() }, async ({ scenario_id }) => {
...
export function registerCustomObjectTools(server: McpServer) {
  ...
  tool(server, "view_custom_object_schema", "View a custom object schema.", { schema_id: z.string() }, async ({ schema_id }) => {
  ...
  tool(server, "view_custom_object_record", "View a custom object record.", { schema_id: z.string(), record_id: z.string() }, async ({ schema_id, record_id }) => {
  ...
  tool(server, "delete_custom_object_record", "Delete a custom object record.", { schema_id: z.string(), record_id: z.string() }, async ({ schema_id, record_id }) => {
...
export function registerFsmTools(server: McpServer) {
  tool(server, "list_service_tasks", "List FSM service tasks.", pageArgs, async ({ page, per_page }) => {
  ...
  tool(server, "view_service_task", "View an FSM service task.", { task_id: z.number().int() }, async ({ task_id }) => {
  ...
  tool(server, "delete_service_task", "Delete an FSM service task.", { task_id: z.number().int() }, async ({ task_id }) => {
    const res = await fd.delete(`/fsm/service_tasks/${task_id}`);
    if (res.status === 204) return text({ success: true });
    return text(errorPayload("Failed to delete service task", res));
  });
  tool(server, "list_appointments", "List FSM appointments.", pageArgs, async ({ page, per_page }) => {
  ...
  tool(server, "view_appointment", "View an FSM appointment.", { appointment_id: z.number().int() }, async ({ appointment_id }) => {
...
export function registerJobTools(server: McpServer) {
  tool(server, "view_job_status", "View bulk job status.", { job_id: z.string() }, async ({ job_id }) => {
...
export function registerAvailabilityTools(server: McpServer) {
  ...
  tool(server, "view_agent_availability", "View an agent's availability.", { agent_id: z.number().int() }, async ({ agent_id }) => {
...
export function registerOmnichannelTools(server: McpServer) {
  tool(server, "list_omnichannel_activities", "List omnichannel activities.", pageArgs, async ({ page, per_page }) => {
