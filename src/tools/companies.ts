...
export function registerCompanyTools(server: McpServer) {
  tool(server, "list_companies", "List companies with pagination.", pageArgs, async ({ page, per_page }) => {
    ...
    return text({
      companies: res.data,
      pagination: { current_page: page, next_page: pagination.next, prev_page: pagination.prev, per_page },
    });
  });

  tool(server, "view_company", "View a company by ID.", { company_id: z.number().int() }, async ({ company_id }) => {
    const res = await fd.get(`/companies/${company_id}`);
    return text(res.ok ? res.data : errorPayload("Failed to fetch company", res));
  });

  tool(server, "search_companies", "Autocomplete companies by name.", { query: z.string() }, async ({ query }) => {
  ...
  tool(server, "find_company_by_name", "Find a company by name.", { name: z.string() }, async ({ name }) => {
  ...
  tool(server, "delete_company", "Delete a company.", { company_id: z.number().int() }, async ({ company_id }) => {
  ...
  tool(server, "view_company_field", "View a company field.", { company_field_id: z.number().int() }, async ({ company_field_id }) => {
  ...
  tool(server, "delete_company_field", "Delete a company field.", { company_field_id: z.number().int() }, async ({ company_field_id }) => {
